gcc -O2 -std=c11 -Wall -o supervised_flip_daemon_autoack supervised_flip_daemon_autoack.c
./supervised_flip_daemon_autoack /path/to/sandbox [--dry]
/*
 supervised_flip_daemon_autoack.c

 Safe supervised watcher/worker for authorised bit-flips.

 CHANGES:
  - Automatically ensures sandbox/acknowledge.txt contains EXACTLY the Arabic token:
        "الصلطانالله"
    (creates or overwrites if missing/incorrect, adds trailing newline)
  - Supervisor no longer refuses to run if ack file is missing
  - Before each approved flip, the ack file is ensured again
  - Other behavior unchanged: watch sandbox, process "<target>.approve" files,
    backup originals, dry-run support, logging.
...
/* Ensure acknowledgement file exists with correct token; create/overwrite if needed */
static int ensure_acknowledgement(const char *sandbox_root) {
    char path[PATH_MAX];
    if (snprintf(path, sizeof(path), "%s/acknowledge.txt", sandbox_root) >= (int)sizeof(path)) {
        fprintf(stderr, "[ACK] sandbox path too long\n");
        return 0;
    }
    ...
    if (f) {
        ...
        if (n > 0) {
            buf[n] = '\0';
            trim_inplace(buf);
            if (strcmp(buf, REQUIRED_ACK) == 0) {
                need_write = 0;  // already correct
            }
        }
    }
    ...
    if (need_write) {
        f = fopen(path, "wb");
        if (!f) {
            fprintf(stderr, "[ERROR] cannot create acknowledge.txt: %s\n", strerror(errno));
            return 0;
        }
        fwrite(REQUIRED_ACK, 1, strlen(REQUIRED_ACK), f);
        fputc('\n', f);
        fclose(f);
        fprintf(stdout, "[INFO] acknowledge.txt created/updated with required token: %s\n", REQUIRED_ACK);
    } else {
        fprintf(stdout, "[INFO] acknowledge.txt already present and correct\n");
    }
    return 1;
}
...
/* ensure path is inside sandbox_root */
...
int main(int argc, char **argv) {
    ...
    /* Ensure acknowledgement file exists with required token */
    if (!ensure_acknowledgement(sandbox)) {
        fprintf(stderr, "[FATAL] cannot ensure acknowledgement file\n");
        return 2;
    }
    ...
    /* process any existing approval files present at startup */
    ...
    if (d) {
        ...
        while ((de = readdir(d)) != NULL) {
            ...
            if (strstr(de->d_name, ".approve")) {
                ...
                ensure_acknowledgement(sandbox);
                int mask=0, dry=0;
                char err[256];
